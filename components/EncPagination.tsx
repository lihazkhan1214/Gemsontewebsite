...
function Page() {
  ...
  // Filter encyclopedia items based on the search term
  const filteredEncyclopediaItems = postData.filter(
    (item) => item.category === 'encyclopedia' && item.title.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const itemsPerPage = 16;
  const totalItems = filteredEncyclopediaItems.length;
  ...
  const currentItems = filteredEncyclopediaItems.slice(indexOfFirstItem, indexOfLastItem);
